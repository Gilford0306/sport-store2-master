...
const getActualOrders = async () => {
  try {
    const response = await fetch("/Order/GetOrdersByUserIdAndStatusId2").then(
      (res) => res.json()
    );
    return (
      response.map(
    ...
    console.error(
      "Error fetching actual orders by statusId=2 (OrderContext):",
      error
    );
    return [];
  }
...
const getAllOrders = async (update = true) => {
  try {
    const ArrayData = await fetch("/Order/GetAllOrdersByUser").then((res) =>
      res.json()
    );
    if (ArrayData)
